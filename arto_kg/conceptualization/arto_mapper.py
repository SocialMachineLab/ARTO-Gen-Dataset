...
class ARTOMapper:
    ...
    def _generate_colour_elements(self, json_data: Dict[str, Any], scene_uri: str, ttl_lines: List[str], validation_data: Optional[Dict[str, Any]] = None):
        ...
        # Build object name → URI map (mirrors _generate_spatial_connectivity)
        obj_name_uri = {}
        enhanced_objects = json_data.get("objects", {}).get("enhanced_objects", [])
        for i, obj_data in enumerate(enhanced_objects):
        ...
        # Path 1: validation step3_color — RGB + VLM-verified name, highest quality
        val_covered: set = set()
        if validation_data:
            try:
        ...
        # Path 3: Text-only primary_colors — skipped for val-covered objects
        for obj_idx, obj_data in enumerate(enhanced_objects):
            obj_name = obj_data.get("name", f"object_{obj_idx}")
            if obj_name.lower() in val_covered:
                continue
