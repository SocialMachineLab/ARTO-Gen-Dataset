...
class ObjectSelector:
    ...
    def select_objects(self, max_secondary_objects: Optional[int] = None, style: str = "Baroque") -> Dict[str, Any]:
        ...
        # Track objects in selection order to correctly handle duplicate COCO IDs.
        # object_metadata (dict) is keyed by ID for _resolve_conflicts compatibility;
        # ordered_meta (list) preserves per-position names so duplicate IDs get distinct names.
        ordered_meta = [{"id": main_id, "specific_name": main_specific_name, "coco_name": main_coco_name}]
        object_metadata = {main_id: {"specific_name": main_specific_name, "coco_name": main_coco_name}}
        for obj in secondary_objects_data:
            ordered_meta.append({"id": obj["id"], "specific_name": obj["specific_name"], "coco_name": obj["name"]})
            if obj["id"] not in object_metadata:
                object_metadata[obj["id"]] = {"specific_name": obj["specific_name"], "coco_name": obj["name"]}

        self.logger.info(f"Initial selection: {[m['specific_name'] for m in ordered_meta]}")

        # Check compatibility - now passes object_metadata so _resolve_conflicts can use variant names
        final_ids, scene_inference = self._resolve_conflicts(main_id, all_ids, style, object_metadata=object_metadata)
        ...
        # Build object_names by consuming from per-ID name queues in selection order.
        # This correctly assigns distinct names when the same COCO ID appears multiple times.
        id_name_queues: Dict[int, List[str]] = {}
        for item in ordered_meta:
            id_name_queues.setdefault(item["id"], []).append(item["specific_name"])

        id_consumed: Dict[int, int] = {}
        object_names = []
        for oid in final_ids:
            idx = id_consumed.get(oid, 0)
            queue = id_name_queues.get(oid, [])
            name = queue[idx] if idx < len(queue) else object_metadata.get(oid, {}).get("specific_name", self.coco_objects.get(oid, str(oid)))
            object_names.append(name)
            id_consumed[oid] = idx + 1
        ...
        return {
            ...
            "primary_object": {
                "id": main_id,
                "name": ordered_meta[0]["coco_name"],
                "specific_name": ordered_meta[0]["specific_name"],
            },
            "secondary_objects": [
                {
                    "id": oid,
