...
class VLLMWrapper:
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            # Automatically adjust configuration based on available GPU count
    ...
    def _adjust_config_for_gpu(self) -> None:
        ...
        # Automatically adjust tensor_parallel_size
        ...
        if gpu_count == 1 and self.config.get('tensor_parallel_size', 1) == 1:
            if 'disable_custom_all_reduce' in self.config:
                # For single GPU, this setting might not be needed
    ...
    def format_chat_prompt(self, system_prompt: str, user_prompt: str) -> str:
        """
        Format system prompt and user prompt into chat template
    ...
    def generate_batch(self, prompts: List[str]) -> List[Dict[str, Any]]:
        """
        Batch generation
        Core advantage feature of vLLM
        """
        if self.model is None:
    ...
    def generate_json_response(self, system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        ...
        # result now includes the result from the unified parser
        enhanced_result = {
        ...
        if result.get("json_data"):
            ...
            try:
                ...
                # If unified parser also failed, fallback to old parsing method
    ...
    def generate_json_response_legacy(self, system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        """
        Old version of generate JSON response method (for backward compatibility)
        """
        # Generate response
        result = self.generate_single(system_prompt, user_prompt)
    ...
    def generate_batch_json_responses(self, prompt_pairs: List[tuple]) -> List[Dict[str, Any]]:
        ...
        # Process results - batch_results now includes parsed results
        json_results = []
        for i, result in enumerate(batch_results):
            if "error" in result:
                ...
                # result already includes the result from the unified parser
    ...
    def test_model_response(self) -> Dict[str, Any]:
        """
        Test model response
        Reuse debugging function from original code
        """
